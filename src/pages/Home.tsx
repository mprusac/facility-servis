...
const Home = () => {
  const services = [
    { icon: Building2, title: "Poslovni prostori", color: "text-primary" },
    { icon: Dumbbell, title: "Sportski objekti", color: "text-secondary" },
    { icon: UtensilsCrossed, title: "Ugostiteljski objekti", color: "text-primary" },
    { icon: HomeIcon, title: "Stambene zgrade", color: "text-secondary" },
  ];
          ...
          <div className="container">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {benefits.map((benefit, index) => (
                <Card 
                  key={index} 
                  className={`text-center hover-lift animate-scale-in animate-delay-${index}00`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {services.map((service, index) => {
                ...
                return (
                  <Card 
                    key={index} 
                    className={`group hover-lift cursor-pointer animate-fade-in-up animate-delay-${index}00`}
              ...
              <CardContent className="pt-8 text-center">
                <div className="flex justify-center mb-4 gap-1">
                  {[...Array(5)].map((_, i) => (
                    <span 
                      key={i} 
                      className={`text-2xl text-secondary animate-scale-in animate-delay-${i}00 inline-block transition-transform hover:scale-125`}
                    >
                      ★
                    </span>
                  ))}
